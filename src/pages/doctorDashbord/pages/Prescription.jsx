...
const durationOptions = ['3 days', '5 days', '7 days', '10 days', '14 days', '21 days', 'As directed'];
...
export default function Prescription({ onClose }) {
  ...
  const printPrescription = () => {
    ...
    const medsContent = selectedMeds
      .map((med, index) => [
  ...
  // SEO metadata
  const pageTitle = patientName ? `كتابة روشتة لـ ${patientName} - لوحة تحكم الطبيب` : 'كتابة روشتة - لوحة تحكم الطبيب';
  const seoDescription = patientName
    ? `كتابة روشتة طبية للمريض ${patientName} بتاريخ ${today} في لوحة تحكم الطبيب باستخدام Clinic Smart.`
    : 'كتابة وإدارة الروشتات الطبية في لوحة تحكم الطبيب باستخدام Clinic Smart.';
  const seoKeywords = `روشتة طبية, لوحة تحكم طبيب, Clinic Smart, أدوية, وصفة طبية${
          ...
          <div className="flex flex-col lg:flex-row gap-3">
            ...
            <div className="w-full lg:w-3/5 bg-white rounded-lg shadow-md p-5">
              ...
              <div className="border border-gray-200 rounded-lg overflow-hidden mb-6">
                <div
                  className="p-3 grid grid-cols-12 gap-1 font-medium"
                ...
                <div className="max-h-64 overflow-y-auto">
                  {selectedMeds.length > 0 ? (
                    selectedMeds.map((med, index) => (
                      <div
                        key={index}
                        className="border-b border-gray-200 p-3 grid grid-cols-12 gap-1 items-center hover:bg-gray-50"
                      >
                    ...
                    {dosageOptions.map((option, index) => (
                      <button
                        ...
                        className={`p-2 border rounded-lg text-sm ${
                          dosage === option ? 'bg-cyan-100 border-cyan-500' : 'border-gray-300 hover:bg-gray-50'
                        }`}
                      >
                        {option}
                      </button>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {durationOptions.map((option, index) => (
                      <button
                        ...
                        className={`p-2 border rounded-lg text-sm ${
                          duration === option ? 'bg-cyan-100 border-cyan-500' : 'border-gray-300 hover:bg-gray-50'
                        }`}
                      >
                        {option}
                      </button>
