        ...
        <div className="card mt-3">
          <div className="card-header bg-white">
            <h5 className="card-title mb-0">
              <i className="bi bi-calendar-check me-2 py-8"></i>
              المواعيد المجدولة
            </h5>
          </div>
                  ...
                  <tbody>
                    {appointments.map((appt, index) => (
                      <tr key={appt.id}>
                        ...
                        <td>{new Date(appt.date).toLocaleDateString('ar-EG')}</td>
                        <td>{appt.time}</td>
                        <td>{appt.notes || <span className="text-muted">لا توجد ملاحظات</span>}</td>
                        <td>
                          <button onClick={() => cancelAppointment(appt.id)} className="btn btn-sm btn-outline-danger">
